...
Here it is important to understand how to count the different possibilities.
Let C(n, k) denote "from n choose k" and [X=k] denote "the number of ways we
can pick exactly k different colored balls". The total number of possibities is
...
(as there are C(7, 2) ways to choose 2 colors from 7). Then it gets more
complicated. For [X=3], we count all partitions of 20 into 3 pieces, with each
partition being between 1 and 10 in size, making sure we do not double count.
Two such partitions are {1, 9, 10} and {6, 6, 7}. For {1, 9, 10} we multiply by
3! to count all permutations, and likewise {6, 6, 7} by 3!/2!. Thus,
